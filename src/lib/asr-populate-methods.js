...
export default class PopulateMethods {
  ...
  createMappingsFromMethodsArray(methodsArray) {
    methodsArray.forEach(item => {
      ...
      if (noNamespace) {
        this.mapNotNamespaced(item, mappingObject, hasAlias, noAlias);
      } else {
        this.mapNamespaced(item, mappingObject, hasAlias, noAlias);
      }
    });
  }
  ...
  mapNotNamespaced(item, mappingObject, hasAlias, noAlias) {
    ...
    if (noAlias && item.binderType === "mutations") {
      ...
      this.vueInstance.$options.methods[item.bind] = mappingObject[item.bind];
      this.reportMapping("noNamespace && noAlias", mappingObject, item);
    }
    ...
    // WITH alias && binderType 'mutation'
      ...
      this.vueInstance.$options.methods[item.alias] = mappingObject[item.alias];
      this.reportMapping("noNamespace && hasAlias", mappingObject, item);
    }
  ...
  mapNamespaced(item, mappingObject, hasAlias, noAlias) {
    ...
    if (noAlias && item.binderType === "mutations") {
      ...
      this.vueInstance.$options.methods[item.bind] = mappingObject[item.bind];
      this.reportMapping("hasNamespace && noAlias", mappingObject, item);
    }
    ...
    // WITH alias && binderType 'mutation'
      ...
      this.reportMapping("hasNamespace && hasAlias", mappingObject, item);
    }
