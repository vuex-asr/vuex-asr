export default class RemoveOldBindings {
    ...
    getComputedKeys(computed) {
        return (computed) ?
            Object.getOwnPropertyNames(computed) : null;

    }

    getMethodKeys(methodKeys) {
        return (methodKeys) ?
            Object.getOwnPropertyNames(methodKeys) : null;
    }


    unSetAsrComputed(vueInstance, computedKeys) {
        if (computedKeys.length > 0 &&
            computedKeys.includes('asrRegisterStart')) {
    ...
    unSetAsrMethods(vueInstance, methodKeys) {
        if (methodKeys.length > 0 &&
            methodKeys.includes('asrRegisterStart')) {
